...
## 0. ★★ v0.3 的定位转向（Yukii 2026-09-24 定）
...
**为什么转向（项目自己的实测证据）**：失败全集中在「分数校准」，不在「发现」。
...
结论：**证据定位、机械验证、问题识别这条链已经工作；坏的恰恰是最后一步"把判断换算成数字"。**
而且这让「教师协作式」从口号变成架构事实 —— 全项目所有层的 `layer_produces_score` 现在统一为 `false`。
...
① L4 逐条定性评估（本层）          findings + judgment/候选档 + 原文定位
        ↓
② 教师（设计，本轮不实现 UI）       三选：准确 / 不准确 / 部分准确；+ 给分（levels 时选最终档）
        ↓
...
**②的设计要点**：教师不做文本修改（太耗时），只在**条目级**对整段评价三选；
「教师没做三选就给分」→ **未标注 ≠ 准确**（评语层当作"AI 初判、未经教师确认"处理，不默认背书）。
...
#### ★★ 输入闸（fail-closed）—— 2026-09-24 修，直接对应 Yukii 的只读验证

旧版只查 `0 ≤ score ≤ 教师文件**自报**的 max`，于是四种篡改全部 `ok=true`。修完的判据：
...
Yukii 指出：「L4 有定位」≠「教师能从最终评语点回原文」。修法 —— 程序把 L4 每条 finding 的定位**回抄**进产物：
...
## 13. 我的判断（区别于 Yukii 的原话，说明在此）

1. **工具 schema 动态生成而非 if/then**：validator 不支持 `not`/`anyOf`，无法表达"levels 条目不许出现 judgment"。
   按条目生成工具（`l4Tools(strategy)`）让"走错分支"物理不可能 —— 比条件约束更强，且 `_selftest` 能逐条钉住。
2. **finding 定位失败不丢 finding**：观察的价值大于引用的价值（丢观察等于把发现藏起来）。
   代价是"带 null 定位的 finding"要严格送审 —— 已落成 `dropped_findings` + `dropped_assessment_ref`。
3. **不设 `certainty` 字段**：项目里模型自报置信度从未被采信（L4 也不用它做准确率）。
   "是否确定"由 **check 挂钩**（可机械核实）与 `severity` 表达 —— 可审计，不靠自述。
4. **`missing_support_evidence` 对 insufficient_evidence / not_applicable 豁免**：
   这两个状态本身就是送审码，再记一条是冗余；且"材料不足"的判断天然无依据。
   （v0.2 stub 断言的相应更新已做 —— 行为变化是**有意的**。）
5. **`levels` 保留 min/max/levels 定义但不再要求模型给分**：档位表是"不许自造档"的机械依据，
   也是教师选档/给分的参照 —— 删掉它反而失去约束。
6. **② 的粒度是条目级**（Yukii 定）：findings 是多条的，逐条三选对教师太贵；
   条目级整段评价三选匹配"30 秒/条目"的预算。
