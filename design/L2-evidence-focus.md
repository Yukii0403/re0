...
**目标模型**：DeepSeek-V4.1-flash —— 1M 上下文 + 多模态。
...
早期把三者混成一个 `kind` 是错的：一个是「要找什么样的话」，一个是「材料以什么形态存在」，
...
**但只有"找得到"还不够，还必须"找对地方"。** 同一句话在原文里出现多次是常态
（重复的页眉、多段重复的套话、多处引用的同一张表）。早期实现用 `indexOf` 取首次出现，
会让所有这些引用都绑到**第一处** —— 证据指向了错误的位置，而且**不会报错**。
...
早期版本里有一条「候选里不允许出现检索面的文字」。**这条是错的，已删除。**

原因：facet 的措辞完全可能是**原文里真实存在的词**。例如 rubric 写「混淆矩阵」，
facet 就是「混淆矩阵结果」，而学生的原文里当然会出现「混淆矩阵」。
拿 facet 措辞去否掉一条合法证据，是把「模型不能说原文」这条规则用错了地方 ——
真正要守的只有一条：**`quote` 必须逐字来自原文**（由 §4.4 的断言保证）。措辞重叠不构成任何问题。

---

## 9. 成本（按 1M 上下文评估）

1. **窗口化暂不需要** —— 整份作业 + 完整 rubric + plan 可一次读完，多数课程作业远小于 1M。
...
```
_l1 无需前置
_realtest        （真实文件测试，产出 fixtures/out/*）
_mkrubric        （需要 design/rubric.example.txt）
_mkplan          （需要 canonical-rubric.example.json）
_mkexample       （需要 plan + rubric；缺 L1 产物会自动补跑 L1 与 page 渲染）
_selftest        （全量回归，缺 L1 产物会自动补跑）
```
...
1. **多文件提交**：已按单文件设计。学生若交「报告 + 代码 + 数据」多份，
   `source_ref.file` 要变多值，且「完整原文」变成多份 —— 下一个要动契约的点。
2. **rubric 的表格输入**：`kind: table` 适配器还没做。表格结构最明确、判据最硬，建议第二个做。
3. **禁用词表**：已降级为辅助（L2 侧目前完全不依赖它，靠 schema 封闭）。
4. **`partial` 的判定门槛**：什么情况下允许只跑部分通道（例如为了省成本跳过语义网）？
   这会影响 recall，需要你定。
5. **成本**（§9）：每个 facet 都要带一次会话，实测一遍真实 usage 再决定要不要合并 facet。
