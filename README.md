...
> 　　★ **建议上传 .txt**（约 5–6 分钟）；**PDF 也可以上传**，但需整页渲染、**耗时更长**（约 6–10 分钟，且内存较小可能失败）。
>
...
**耗时（务必先读）**：**建议上传 .txt**（线上实测约 **5–6 分钟**）；**PDF 也可以上传**，但需要把整页渲染成图，**耗时更长**（约 **6–10 分钟**），
且免费档内存较小，PDF 在渲染阶段**有可能失败** —— 失败时请改用 TXT，或直接看下面的兜底演示站（含 PDF 案例）。
