...
>
> **三案例演示页（含新 rubric 案例）**：https://autograder-sotv.onrender.com/
> 　　这是预生成结果，不再次调用模型；但与实时站共用 Render 免费实例，可能有冷启动。
> **旧静态快照（不自动同步 GitHub）**：https://0795576dbd8c403cb6668f07a7ecbf4e.app.workbuddy.host
> 　　无冷启动，可作旧版 PDF 案例兜底；不应把它说成已包含新 rubric 案例。
...
| **全新 .txt 报告 + 全新 TXT rubric** | **148 秒**（约 2.5 分钟） | 合成哈希索引实验报告，4 个评分项；公网真实模型 L1→L4 与教师视图完成，具体耗时不保证复现 |
...
https://autograder-sotv.onrender.com/ —— 当前三案例演示页：新增合成报告 + 新上传 rubric 的真实模型产物，另含旧版 PDF 与 VerAs 案例。页面内容为预生成 HTML，但 Render 免费实例有冷启动。
旧 WorkBuddy 地址是独立静态快照，**不会随 GitHub 推送自动更新**。
...
**当前三案例演示页**：Render 服务的 `/` 直接读取仓库 `fixtures/web/`；代码推送并完成 Render 部署后更新。
旧 WorkBuddy CloudStudio 地址是另一次独立的静态托管快照，需在该平台单独重新发布，不能以 GitHub push 或 Render 更新代替。
`fixtures/web/` 整个目录也可作为其他静态托管的站点根（`index.html` 为入口）。
